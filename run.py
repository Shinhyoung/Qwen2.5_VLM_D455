...
def run_inference(model, processor, pil_image: Image.Image,
    ...
    """PIL 이미지를 VLM 모델로 분석하고 결과 텍스트를 반환한다."""
    messages = [{
        "role": "user",
        "content": [
...
def build_result_image(frame_bgr: np.ndarray, result_text: str,
    ...
    """캡처 이미지와 분석 결과 텍스트를 하나의 이미지로 합성한다."""
    padding         = 20
    line_height     = 24
    text_font_size  = 16
    title_font_size = 18
    ...
    text_area_h     = len(lines) * line_height + 2 * padding + 32
...
def main():
    ...
    is_analyzing    = False
    pending_result  = None  # worker → main: (frame_bgr, result_text)
    ...
    def analysis_worker(frame_bgr: np.ndarray):
        ...
        try:
            text = run_inference(model, processor, pil_image)
        except torch.cuda.OutOfMemoryError:
            text = "오류: GPU 메모리가 부족합니다."
        except Exception as e:
            text = f"분석 오류: {e}"
        ...
        with lock:
            pending_result = (frame_bgr.copy(), text)
        ...
        while True:
            ...
            with lock:
                if pending_result is not None:
                    result_image   = build_result_image(*pending_result)
                    pending_result = None
